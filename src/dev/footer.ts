...
Callback.addCallback("PostLoaded", () => {
    ...
    ["fe", "rf", "mj", "tesla", "eu1", "eu2", "eu3", "eu4", "eu5"].forEach(name => {
...
ModAPI.addAPICallback("ICore", () => {
    ["LV", "MV", "HV", "EV"].forEach((name, index) => {
        const transformerID = `transformer${name}`;
        const producerID = `energy_producer_eu${index + 1}`;
